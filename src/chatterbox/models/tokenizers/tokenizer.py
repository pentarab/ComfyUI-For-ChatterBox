...
class ChineseCangjieConverter:
    ...
    def _load_cangjie_mapping(self, model_dir=None):
        """Load Cangjie mapping from local model directory."""
        try:
            if model_dir is None:
                logger.warning("No model directory provided for Cangjie mapping")
                return

            cangjie_file = Path(model_dir) / "Cangjie5_TC.json"

            if not cangjie_file.exists():
                logger.warning(f"Cangjie mapping file not found: {cangjie_file}")
                return
